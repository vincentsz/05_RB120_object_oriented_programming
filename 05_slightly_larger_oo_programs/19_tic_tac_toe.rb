...
class TTTGame
  ...
  def initialize
    @board = Board.new
    @human = Player.new(Board::DEFAULT_MARKER)
  ...
  def pick_marker(choice = Board::DEFAULT_MARKER)
    loop do
    ...
    board.custom_marker = if YES.include?(answer)
                            choose
  end
  ...
  #     break if custom_marker.size == 1 && custom_marker != Board::COMPUTER_MARKER
  #     puts "Invalid input!"
  #   end
  #   human.marker = custom_marker
  #   board.custom_marker = custom_marker
  ...
  def pick_beginning_player
    first = ''
    loop do
      puts "Choose who goes first! ('p' for player, 'c' for computer, 'r' for random)"
  ...
  def set_beginning_player(first)
    if VALID_P.include?(first)
      ...
      @current_marker = [human.marker, Board::COMPUTER_MARKER].sample
    end
  end
