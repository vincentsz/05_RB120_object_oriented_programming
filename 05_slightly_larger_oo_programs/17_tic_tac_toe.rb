...
class TTTGame
  ...
  def pick_marker
    ...
    loop do
      ...
      puts "'O' is resereved for the computer. "
  ...
  def set_beginning_player(first)
    if VALID_P.include?(first)
      @current_marker = board.human_marker
    elsif VALID_C.include?(first)
      @current_marker = Board::COMPUTER_MARKER
    else
      @current_marker = [board.human_marker, Board::COMPUTER_MARKER].sample
    end
  end
