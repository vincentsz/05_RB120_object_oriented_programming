class Board
  ...
  # def computer_mark_in_line(line)
  #   line.any? {|num| @squares[num] == TTTGame::COMPUTER_MARKER }
  # end

  # def computer_moves
  #   if board.immediate_treath?
  #     board[board.treath] = computer.marker#pick immeditae_treath
  #   else
...
class TTTGame
  ...
  def computer_moves
    if immediate_move?(COMPUTER_MARKER)
      board[move(COMPUTER_MARKER)] = computer.marker
    elsif immediate_move?(HUMAN_MARKER)
      board[move(HUMAN_MARKER)] = computer.marker
    else
      board[board.unmarked_keys.sample] = computer.marker
    end
  end
  ...
  def move(marker)
    Board::WINNING_COMBINATIONS.each do |line|
      next unless board.one_unmarked_key?(line)
      markers = line.map {|num| board.squares[num].marker}
      return line[markers.index(Square::INITIAL_MARK)] if markers.count(marker) == 2
    end
    nil
  end
