...
class Deck
  ...
  def deal(hand, num)
    num.times do
      hand.cards << @cards.sample
      @cards.delete(hand.cards.last)
    end
  end
end
...
class Game
  ...
  def player_turn
    loop do
      ...
      player_moves
      return @player.display_bust if @player.hand.busted
    end
    @player.stay
  end
  ...
  def player_moves
    ...
    @player.show_hand
    @player.hand.total_points
    @player.hand.handle_bust
  end

  def computer_turn
    @computer.show_hand
    loop do
      break if @computer.hand.points >= Computer::MINIMUM 
      handle_screen(4)# && @computer.hand.points > @player.hand.points (more competitive dealer)
      computer_moves
      return @computer.display_bust if @computer.hand.busted
    end
    handle_screen(4)
    @computer.stay
  end
