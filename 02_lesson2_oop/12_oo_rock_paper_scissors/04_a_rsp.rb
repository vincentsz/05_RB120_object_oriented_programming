...
# What data structure will you reach for? Will you use a new class, or an existing class? What will the display output look like?
# -------- Classes ----------
class Player
  attr_accessor :choice, :name, :score, :moves
...
class Human < Player
  ...
  def choose
    ...
    self.choice = Choice.new(temp_choice, moves)
  end
...
class Computer < Player
  ...
  def choose
    self.choice = Choice.new(Choice::CHOICES.sample, moves)
  end
end
...
class RPSGame
  ...
  def display_moves
  puts "#{human.name} has played: #{human.moves.join(', ')}."
  puts "#{computer.name} has played: #{computer.moves.join(', ')}."
  end
